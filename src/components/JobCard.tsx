...
export default function JobCard({
  ...
  return (
    <Link
      href={`/jobs/${job.slug}`}
      className="group flex items-start gap-4 rounded-2xl border border-line bg-white/70 p-5 transition hover:-translate-y-0.5 hover:border-gold hover:shadow-[0_14px_32px_rgba(44,57,71,0.08)]"
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h3 className="mt-0.5 truncate font-display text-lg font-bold text-ink transition-colors group-hover:text-gold">
              {job.title}
            </h3>
            {job.employer?.name && (
